...
void init(cmplx* const psi0, const double alpha, const double lambda, const double dx, const double dt,
...
void writeToFile(const cmplx* const v, const string s, const double dx, const int Nx, const double xmin, 
	const double alpha, const double lambda, const double omega, const double t);

void step(cmplx* const psi1, cmplx* const psi0, double* V, const double m, const double h_quer, 
...
int main() {
	...
	const double m = 1;
	const double h_quer = 1;
	const double k = omega*omega*m;
	const double alpha = pow(m*k/h_quer, 1/4.0);
	...
	for (int i = 1; i <= Na; i++) {
		for (int j = 1; j <= Nk-1; j++) {
		    ...
		    step(psi1, psi0, V, m, h_quer, dt, dx, Nx, xmin);
...
void step(cmplx* const psi1, cmplx* const psi0, double* V, const double m, const double h_quer, 
	...
	cmplx a = - cmplx(0,1)*h_quer*dt/(4*m*dx*dx);

	for(int i = 0; i < Nx; i++) 
		d[i] = 1.0 + cmplx(0,1)*(h_quer*dt/(2*m*dx*dx) + dt*V[i]/(2*h_quer));
	...
	for(int i = 1; i < Nx; i++){
		d[i] = d[i] - a*a/d[i-1];		//da d_schlange[0] = d[0]
